...
| Gemini | `gemini` | `-m <model> -y --output-format text` | `gemini-2.5-flash`, etc. Personality baked into prompt body (no `--system-prompt` flag in Gemini CLI). **Headless dispatch requires `GEMINI_API_KEY`** — the Gemini CLI's OAuth login flow is interactive and cannot complete inside a non-TTY daemon child. Set the key under `[agents.env]` in `~/.crosstalk/config.toml`, in `~/.gemini/.env`, or in the daemon's shell env. If only OAuth credentials exist (`~/.gemini/oauth_creds.json`), spawned dispatches will hang on a login prompt. Interactive `channel join --agent gemini` is unaffected — it inherits the parent TTY and can drive OAuth normally. **⚠ Sunsets 2026-06-18** — Google retires the Gemini CLI on that date in favor of [Antigravity CLI](https://antigravity.google/product/antigravity-cli) (see row below). Existing actor configs continue to dispatch via `gemini` until then; plan to migrate before the cutover. |
| Antigravity | `agy` | _native dispatch integration planned, not yet implemented — track via the `agent: antigravity` actor type that will land in v1.14+_ | Google's official successor to the Gemini CLI ([repo](https://github.com/google-antigravity/antigravity-cli)). Built in Go; shared agent engine with the Antigravity 2.0 desktop app. Today the CLI is install-via-curl and TUI-first; the headless `-p / --prompt` flag exists but the exact non-interactive auth path is unclear (the README documents Google-Sign-In via system keyring, no native API-key fallback). For now: use Antigravity interactively via `channel join --agent agy` (operator drives the TTY auth); native daemon dispatch is deferred to a v1.14 design pass once Google publishes the headless reference. Gemini remains the supported daemon-dispatch path through the 2026-06-18 sunset. |
