...
- [ ] **BUG (v1.0.x candidate) — heartbeat-interval timeout doesn't kill opencode subprocesses.** Surfaced by Monte Carlo π 20-way v1.0.1 re-validation (2026-05-17): dart-thrower-19 + dart-thrower-20 were still running **56 minutes** after dispatch despite `heartbeat-interval: 60` in their profiles. Manually SIGTERMed at the 56-min mark — would have run indefinitely otherwise. `dispatch.ts` sets a `setTimeout` that calls `proc.kill()` at the interval, but SIGTERM to opencode's wrapper doesn't propagate to its subprocess tree (the underlying LLM client process). Result: slow/hung LLM calls leak processes **indefinitely** until manually killed — accumulating memory, CPU, and OpenRouter connection slots. In long-running production swarms this would compound daily. **Severity: higher than I initially scoped it** — this isn't just a slow-LLM hygiene issue, it's a steady-state resource leak. **Fix:** use `process.kill(-proc.pid, 'SIGKILL')` to kill the entire process group, or detect + clean up orphaned children explicitly. File against `src/dispatch.ts` heartbeat-timeout path.
